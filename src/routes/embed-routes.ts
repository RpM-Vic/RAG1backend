...
import {
  calculateLllmCallPriceInCredits,
  llmCentsPerMillionTokens,
} from '../models/pricesLLMs.js';
...
embedBook.post('/', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    for (
      let currentPageNum = startFromPage;
    ...
    const bookUpdated = await updateBookById(book_id, {
      embedded_pages: totalPages,
    });
    const newBalance = await reduceCredits(
      user.credits - userCreditsBuffer,
      user_id,
    );
